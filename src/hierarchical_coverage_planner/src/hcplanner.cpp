...
void hierarchical_coverage_planner::HCPlan(ros::NodeHandle &nh) {
    ...
    Eigen::Vector3i xyz(
        (HCMap->hcmp_->map_max_boundary_(0) - HCMap->hcmp_->map_min_boundary_(0)) / HCMap->hcmp_->resolution_,
    ...
    for (int i = 0; i < (int)PR.ori_model->points.size(); ++i)
        voxelMap.setOccupied(
            Eigen::Vector3d(PR.ori_model->points[i].x, PR.ori_model->points[i].y, PR.ori_model->points[i].z));
    ...
    *Fullmodel = *PR.occ_model;
    ...
    ROS_INFO("\033[36m[CoverageAnalyzer] FC-Planner path coverage evaluation time = %lf s. \033[32m",
             cove_time / 1000.0);
...
void hierarchical_coverage_planner::CoveragePlan(bool turn) {
    ...
    auto plan_t1 = std::chrono::high_resolution_clock::now();
    tuple<map<int, vector<Eigen::VectorXd>>, map<int, vector<vector<Eigen::VectorXd>>>> LCPResults =
        solver_->LocalConditionalPath(PR.final_sub_vps_pairs, PR.global_boundary_id_, turn);
    ...
    // * Convert Global Path
    tuple<vector<Eigen::VectorXd>, vector<vector<Eigen::VectorXd>>> FullResults =
        solver_->CoverageFullPath(current_pos_, PR.global_seq_, PR.local_paths_viewpts_, PR.local_paths_waypts_);
    ...
    for (auto sub_space : skeleton_operator->P.sub_space_scale) {
                ...
                if (zFlag == true) {
                    if (pt_.z > skeleton_operator->groundHeight) {
                        ...
                        // for safety of viewpoints under ground constraint
                            ...
                            // new safe direction
                            double z_new = sqrt(pow(new_dir(2), 2) * (pow(normal_dir(0), 2) + pow(normal_dir(1), 2)) /
                                                (pow(new_dir(0), 2) + pow(new_dir(1), 2)));
        ...
        for (auto vp : seg_vps->points) {
            ...
            for (int i = 0; vp.x - safe_radius_vp + i * HCMap->hcmp_->resolution_ < vp.x + safe_radius_vp; i = i + 2)
                for (int j = 0; vp.y - safe_radius_vp + j * HCMap->hcmp_->resolution_ < vp.y + safe_radius_vp;
                     j = j + 2)
                    for (int k = 0; vp.z - safe_radius_vp + k * HCMap->hcmp_->resolution_ < vp.z + safe_radius_vp;
                         k = k + 2) {
...
bool hierarchical_coverage_planner::lightStateDet(Eigen::Vector3d &vpCandi, Eigen::Vector3d &vpCandiNormal) {
    ...
    for (int i = 0; i < K; ++i) {
        ...
        nearestROSAVertex << rosaCloud->points[nearest_id[i]].x, rosaCloud->points[nearest_id[i]].y,
            rosaCloud->points[nearest_id[i]].z;
...
void hierarchical_coverage_planner::cutting_plane_normals() {
    ...
    for (const auto &pt_normal : PR.pt_normal_pairs) {
        ...
        oPNormal.resize(6);
        oPNormal << pt_normal.first(0), pt_normal.first(1), pt_normal.first(2), pt_normal.second(0),
            pt_normal.second(1), pt_normal.second(2);

        PR.outer_normals.push_back(oPNormal);
    }
}

void hierarchical_coverage_planner::plane_query(pcl::PointCloud<pcl::PointXYZ>::Ptr &point_cloud,
                                                Eigen::Vector3d &point_on_plane, Eigen::Vector3d &plane_normal,
                                                double thickness) {
...
void hierarchical_coverage_planner::JointRefinePath(bool turn) {
    ...
    for (auto x : JointId) {
        ...
        JointPos << skeleton_operator->P.vertices_scale.row(x)(0), skeleton_operator->P.vertices_scale.row(x)(1),
            skeleton_operator->P.vertices_scale.row(x)(2);
    ...
    ROS_INFO("\033[37m[LocalRefine] initial path length = %lf m, refined path length = %lf m.\033[32m", oriLength,
             refineLength);
...
double hierarchical_coverage_planner::CoverageEvaluation(vector<Eigen::VectorXd> &poseSet) {
    ...
    ROS_INFO("\033[36m[CoverageAnalyzer] FC-Planner trajectory coverage evaluation time = %lf s.\033[32m",
             ce_time / 1000.0);
...
void hierarchical_coverage_planner::CameraModelProjection(Eigen::VectorXd &pose, Eigen::Vector3d &point, double &xRes,
                                                          double &yRes, Eigen::Vector2d &leftdown, double &distance,
                                                          Eigen::Vector2i &inCam) {
